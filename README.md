...
| isSaveTokensInUserDefaults | Bool | Сохранять ли токен юзера ("Token", смотреть выше) в UserDefaults приложения. 
При занчении "True" позволяет не хранить токены в ваших системах, но он будет храниться только в текущем устройстве. 
Для доступа к переписке клиента с других устройств и платформ необходимо хранить токен в вашей системе и передавать его при инициализации. В этом случае значение параметра должно быть "False" |
